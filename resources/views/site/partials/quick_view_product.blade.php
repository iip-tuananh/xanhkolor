    ...
    <div class="row">
        <div class="product-left-column product-images col-xs-12 col-sm-4 col-md-4 col-lg-5 col-xl-4">
                ...
                <span class="view_full_size">
                    <a class="img-product" title="Ảnh sản phẩm" href="javascript:;">
                        <img src="{{ $product->image ? $product->image->path : '' }}"
                            id="product-featured-image-quickview" class="img-responsive product-featured-image-quickview"
                            alt="quickview">
                    </a>
                </span>
            ...
            <div class="more-view-wrapper clearfix">
                <div class="thumbs_quickview owl_nav_custome1 swiper-container thumbs_list_quickview swiper-container-initialized swiper-container-horizontal swiper-container-pointer-events"
                    ...
                    <ul class="product-photo-thumbs quickview-more-views-owlslider not-thuongdq swiper-wrapper"
                        ...
                        @foreach ($product->galleries as $gallery)
                            <li class="swiper-slide active swiper-slide-active"
                                style="width: 78.6667px; margin-right: 10px;">
                                <a href="javascript:void(0)"
                                    data-imageid="{{ $gallery->id }}" data-zoom-image="{{ $gallery->image ? $gallery->image->path : '' }}">
                                    <img src="{{ $gallery->image ? $gallery->image->path : '' }}"
                                        alt="Ảnh sản phẩm" style="max-width:120px; max-height:120px;"></a>
                            </li>
                        @endforeach
                    </ul>
                        ...
                        <div class="onirvapp--shape-xs">
                            <div class="onirvapp--shape-container">
                                ...
                                <div class="onirvapp--shape-solid"
                                    style="width: {{ $product->product_rates->count() > 0 ? (round($product->product_rates->sum('rating') / $product->product_rates->count()) / 5) * 100 : 0 }}%">
                                </div>
                            </div>
                        </div><span class="onireviewapp-loopitem-title">({{ $product->product_rates->count() }} đánh
                            giá)</span>
                    </div>
                </div> --}}
                <div class="vend-qv group-status">
                    <div class="left_vend">
                        <div class="first_status ">Mã sản phẩm:
                            <span class="vendor_ status_name">{{ $product->sku }}</span>
                        </div>
                        <span class="line_tt">|</span>
                        <div class="top_sku first_status">Tình trạng:
                            <span class="sku_ status_name"><span>Còn hàng</span></span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="quickview-info">
                @if ($product->base_price > 0 && $product->price > 0)
                    <span class="prices price-box">
                        ...
                        <del class="old-price">{{ formatCurrency($product->base_price) }}₫</del>
                    </span>
                @elseif ($product->base_price > 0 && $product->price == 0)
                    <span class="prices price-box">
                        <span class="price product-price sale-price">{{ formatCurrency($product->base_price) }}₫</span>
                    </span>
                @else
                    <span class="prices price-box">
                        <span class="price product-price sale-price">{{ formatCurrency($product->price) }}₫</span>
                ...
                <div class="form_product_content">
                    <div class="count_btn_style quantity_wanted_p">
                            ...
                            <div class=" soluong1 clearfix">
                                <div class="input_number_product">
                                    ...
                                    <input type="text" id="quantity-detail" name="quantity"
                                        ng-model="quantity_quickview" maxlength="2" class="form-control prd_quantity"
                        ...
                        <div class="button_actions clearfix">
                            @if (!$product->type)
                                <button type="button"
                                    class="btn_cool btn btn_base fix_add_to_cart ajax_addtocart btn_add_cart btn-cart"
                                    ng-click="addToCart({{ $product->id }}, quantity_quickview)">
                                    <span class="btn-content text_1">Thêm vào giỏ hàng</span>
                                </button>
                            @else
                                <button type="button"
                                    class="btn_cool btn btn_base fix_add_to_cart ajax_addtocart btn_add_cart btn-cart"
                                    onclick="window.open('{{ $product->type ? $product->short_link ?? $product->aff_link : route('front.show-product-detail', $product->slug) }}', '_blank')">
